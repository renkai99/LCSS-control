    ...
    for j=1:num_lat
        for t=1:T
            ...
            constraints_obs = [constraints_obs , ...      
                mean(-coeff1) * temp_x + r1 * sqrt(temp_x' * cov(-coeff1) * temp_x) + sqrt(1+r2) * sqrt(temp_x' * cov(-coeff1) * temp_x) * norminv(1-eps_ijt) + 1/2 * CAR_R - big_M*(1-delta(1,t,i,j)') <= 0 ,...
                mean(-coeff2) * temp_x + r1 * sqrt(temp_x' * cov(-coeff2) * temp_x) + sqrt(1+r2) * sqrt(temp_x' * cov(-coeff2) * temp_x) * norminv(1-eps_ijt) + 1/2 * CAR_R - big_M*(1-delta(2,t,i,j)') <= 0 ,...
                mean(-coeff3) * temp_x + r1 * sqrt(temp_x' * cov(-coeff3) * temp_x) + sqrt(1+r2) * sqrt(temp_x' * cov(-coeff3) * temp_x) * norminv(1-eps_ijt) + 1/2 * CAR_R - big_M*(1-delta(3,t,i,j)') <= 0 ,...
                mean(-coeff4) * temp_x + r1 * sqrt(temp_x' * cov(-coeff4) * temp_x) + sqrt(1+r2) * sqrt(temp_x' * cov(-coeff4) * temp_x) * norminv(1-eps_ijt) + 1/2 * CAR_R - big_M*(1-delta(4,t,i,j)') <= 0 ,...
            ];
