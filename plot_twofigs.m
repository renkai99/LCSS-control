...
h = figure();
set(h, 'Position', [100, 100, 600, 290], 'Color', 'w')
...
for i = 1 : N_frames
    ...
    if scene == 1
        ax.Position = [left-0.03 bottom+0.16 0.43 0.76];
    else
        ax.Position = [left-0.03 bottom+0.02 0.42 1.04];
    end
    ax.FontSize = 11;
    
    if i == N_frames
        legend({'MTA','MRA','CVaR','CVaRR'}, 'Location', 'southoutside', 'Orientation', 'horizontal', 'FontSize', 11);
        Lgnd = legend('show');
        Lgnd.Position(1) = 0.2;
        Lgnd.Position(2) = 0.03;
        
        % MagInset for two scenes
        if scene == 1
            MagInset(h, hs, [865 870.5 1421 1426], [885 895 1397 1407], {'SW','NW';'SE','NE'}); grid off; legend('hide'); set(gca,'xtick',[]); set(gca,'ytick',[]); set(gca,'XTickLabel',[]); set(gca,'YTickLabel',[]);
        elseif scene == 2
            MagInset(h, hs, [1215.8 1220 958.3 961.2], [1207 1220 970 979], {'NW','SW';'NE','SE'}); grid off; legend('hide'); set(gca,'xtick',[]); set(gca,'ytick',[]); set(gca,'XTickLabel',[]); set(gca,'YTickLabel',[]);
        end
    end
end