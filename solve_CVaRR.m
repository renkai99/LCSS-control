function [ u_star, cost, x_star, DIAGNOSTIC ] = solve_CVaRR(params, OV, EV, eps_assign)
%%  Load needed parameters
car.d = params.car.d ;
CAR_L = sqrt(car.d(1)^2 + car.d(2)^2); % car.d(1);
...
for i=1:num_OV
    ...
    for j=1:num_lat
        for t=1:T
            ...
            N_s = length(OV{i}.pred_yaw{j}(:,t));
            Tval = (nd*(N_s-1)) / (N_s-nd) * finv(1-beta_r1,nd,N_s-nd); % try fpdf()
            r1 = sqrt(Tval/N_s);
            r2 = max(abs(((N_s-1)/chi2inv(beta_r2/2,N_s-1)-1)),abs(abs(((N_s-1)/chi2inv(1-beta_r2/2,N_s-1)-1))));
            eps_ijt = eps_assign(i, j)/T;

            constraints_obs = [constraints_obs , ...      
                mean(-coeff1 * temp_x + 1/2 * CAR_L) + r1 * sqrt(temp_x' * cov(-coeff1) * temp_x) + sqrt(1+r2) * sqrt(var(-coeff1 * temp_x + 1/2 * CAR_L))*((normpdf(norminv(1-eps_ijt)))/eps_ijt) - big_M*(1-delta(1,t,i,j)') <= 0 ,...
                mean(-coeff2 * temp_x + 1/2 * CAR_L) + r1 * sqrt(temp_x' * cov(-coeff2) * temp_x) + sqrt(1+r2) * sqrt(var(-coeff2 * temp_x + 1/2 * CAR_L))*((normpdf(norminv(1-eps_ijt)))/eps_ijt) - big_M*(1-delta(2,t,i,j)') <= 0 ,...
                mean(-coeff3 * temp_x + 1/2 * CAR_L) + r1 * sqrt(temp_x' * cov(-coeff3) * temp_x) + sqrt(1+r2) * sqrt(var(-coeff3 * temp_x + 1/2 * CAR_L))*((normpdf(norminv(1-eps_ijt)))/eps_ijt) - big_M*(1-delta(3,t,i,j)') <= 0 ,...
                mean(-coeff4 * temp_x + 1/2 * CAR_L) + r1 * sqrt(temp_x' * cov(-coeff4) * temp_x) + sqrt(1+r2) * sqrt(var(-coeff4 * temp_x + 1/2 * CAR_L))*((normpdf(norminv(1-eps_ijt)))/eps_ijt) - big_M*(1-delta(4,t,i,j)') <= 0 ,...
                ];
