...
scene = forecast.sc; % 1 if scene103_t14 / 2 if scene556_t6
    
if scene == 1
    v_ego = 1;
    position_ego = forecast.vehicles(v_ego).past(end,:);
    position_prev_ego = forecast.vehicles(v_ego).past(end-2,:);
    yaw_ego = atan2(position_ego(2)-position_prev_ego(2), position_ego(1) - position_prev_ego(1));
    vel_ego = norm(position_ego-position_prev_ego)/dt;
    EV.p0 = position_ego;
    EV.v0 = vel_ego;
    EV.yaw0 = yaw_ego;
    EV.Rot = [cos(yaw_ego), -sin(yaw_ego); sin(yaw_ego) cos(yaw_ego)];
    EV.past = forecast.vehicles(v_ego).past;
elseif scene == 2
...
% v_interest = [6, 8, 9]; %for scene103_t10
if scene == 1
    v_interest = [4, 7]; %for scene103_t14
elseif scene == 2
...
%     figure(1)
%     hold on; axis equal;
%     plot(past(:,1), past(:,2), 'o', 'MarkerFaceColor', v_color(j))
    if scene == 2
