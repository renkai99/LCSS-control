...
if scene == 1
    fileName = './data/predict_scene103_t14_data_2/predict_scene103_t14_meta_7000.json'; % filename in JSON extension
elseif scene == 2
    fileName = './data/predict_scene556_t6_data/predict_scene556_t6_meta_7000.json'; % filename in JSON extension
elseif scene == 3
    fileName = './data/predict_scene105_t11_data/predict_scene105_t11_meta_50000_3.json'; % filename in JSON extension
end
...
[u_proposed, cost_proposed, car_states_proposed, A_union, b_union, t_overapprox, w_vertex, DIAGNOSTIC_proposed] = solve_proposed(params, OV, EV);
...
for i=1:O
    for t=1:T
        ...
        H1 = A_union{t, clu, i};
        h1 = b_union{t,clu,i};
        v_overapprox{t, clu, i} = Polyhedron(H1, h1).V;
        p_overapprox{t, clu, i} = polyshape([v_overapprox{t, clu, i}(:,1); v_overapprox{t, clu, i}(1,1)], [v_overapprox{t, clu, i}(:,2); v_overapprox{t, clu, i}(1,2)]);
        end
    end
end

%% Reproduce Paper Results
% [emp_rate_viol_MTA, emp_viol_amt_MTA] = montecarlo_empRA(params, OV, car_states_MTA, eps_assign);
% [emp_rate_viol_MRA, emp_viol_amt_MRA] = montecarlo_empRA(params, OV, car_states_MRA, eps_assign);
% [emp_rate_viol_CVaR, emp_viol_amt_CVaR] = montecarlo_empRA(params, OV, car_states_CVaR, eps_assign);
[emp_rate_viol_CVaRR, emp_viol_amt_CVaRR] = montecarlo_empRA(params, OV, car_states_CVaRR, eps_assign);
[emp_rate_viol_pp, emp_viol_amt_pp] = montecarlo_empRA(params, OV, car_states_proposed, eps_assign);
...
for i = 1 : N_frames
    ...
    % place the map image
    if scene == 1
        map = imread('./data/predict_scene103_t14_data_2/predict_scene103_t14_map.png', 'png', 'BackgroundColor', 'none');
    elseif scene == 2   
        map = imread('./data/predict_scene556_t6_data/predict_scene556_t6_map2.png', 'png', 'BackgroundColor', 'none');
    elseif scene == 3
        map = imread('./data/predict_scene105_t11_data/predict_scene105_t11_map2.png');
    end
