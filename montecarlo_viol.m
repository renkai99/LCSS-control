function [rate_viol, viol_amt] = montecarlo_viol(params, OV, x_star)
...
num_viol = zeros(num_OV, max(K));
rate_viol = zeros(num_OV, max(K));
viol_amt = zeros(num_OV, max(K));
num_mc = 1e5;
...
for i=1:num_OV
    ...
    for j=1:num_lat
        for t=1:T
            disp(['OV', num2str(i), ', Latent', num2str(j), ', Time', num2str(t)])
